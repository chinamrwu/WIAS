...
for(i in 1:3){
  for(j in (i+1):4){
	label <- c(substr(Labels,i,i),substr(Labels,j,j))
	bestSample=c()
	bestTest=c()
	bestModel=list()
	bestFeatures=c()
	bestPerformance <- 0;
		...
		indexA <- sample(which(SWT$label==substr(Labels,i,i)),testNumber[i],replace=F)
		indexC <- sample(which(SWT$label==substr(Labels,j,j)),testNumber[j],replace=F)
