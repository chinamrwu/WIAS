...
LearningCurve <- function(dat,
  ...
  Labels <- unique(dat[,outcome])
  Index01 <- which(dat[,outcome]==Labels[1])
  Index01 <- which(dat[,outcome]==Labels[2])
  ...
  for(i in seq(along = proportion)) {
    if(verbose) cat("Training for ", round(proportion[i]*100, 1),"% (n = ", floor(n*proportion[i]), ")\n", sep = "")
