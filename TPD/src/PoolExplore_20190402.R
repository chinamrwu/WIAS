...
ump30 <- drawUMAP(pool[,R0 <= 30],color3,strTitle="UMAP:pool samples with 30% missing rate",rowNormalization=T,colNormalization=T)
ump30 <- ump30 + scale_x_continuous(breaks = round(seq(min(ump30$data$X), max(ump30$data$X), by = 0.5),1))+ 
     scale_y_continuous(breaks = round(seq(min(ump30$data$Y), max(ump30$data$Y), by = 0.5),1)) 
dat <- ump30$data
rowsA <- rownames(dat)[dat$X > 5]
rowsB <- rownames(dat)[dat$Y < -5 & dat$X > -4.2 & dat$X < -2.2]
rowsC <- rownames(dat)[dat$Y < -5 & dat$X < -5]
...
f1 <- function(r1,r2){
   d1 <- scaleRow(pool[r1,])
	...
	d2$label <- 'B'
	d0 <- rbind(d1,d2)
	d0$label <- as.factor(d0$label)
	d0[is.na(d0)] <- 0
	rf0 <- randomForest(label ~ . ,data=d0,importance=T,ntree=500,nodesize=5)
   imps  <- data.frame(importance(rf0))
   imps <- imps[order(imps$MeanDecreaseAccuracy,decreasing=T),]
	rownames(imps)[imps$MeanDecreaseAccuracy==0]
}

prs <- f1(rowsA,rowsB)
prs <- unique(prs,f1(rowsA,rowsC))
prs <- unique(prs,f1(rowsB,rowsC))

#tmp <- pool[,!colnames(pool) %in% prs]
tmp <- pool[,c('label',prs)]

umpK <- drawUMAP(tmp,color3,strTitle="UMAP:pool samples with 50% missing rate RF filtered",rowNormalization=T,colNormalization=T)
