...
protIds <- colnames(t0)[!grepl('iRT',colnames(t0))]
...
write.table(t0,file="E:/projects/TPD/data/TPD_prot_matrix_rep_20190131.txt",sep="\t",col.names=T,row.names=T,quote=F)
...
t1 <- t1[,c('SpecimenID','label',protIds)]
L=dim(t1)[1]
rownames(t1) <- 1:L

write.table(t1,file="E:/projects/TPD/data/TPD_prot_matrix_avg_20190131.txt",sep="\t",col.names=T,row.names=F,quote=F)
...
p1 <-  drawPCA(t2,c(training='#537e35',validation ='#e17832'),colNormalization=T)
p2 <-  drawPCA(t2,c(training ='#537e35',validation ='#e17832'),colNormalization=F,rowNormalization=T)
p3 <- drawPCA(t2,c(training ='#537e35',validation ='#e17832'),colNormalization=T,rowNormalization=F)
p4 <- drawPCA(t2,c(training ='#537e35',validation='#e17832'),colNormalization=T,rowNormalization=T)

pdf("E:/projects/TPD/results/PCA_SWATH_505protein_20190131.pdf",width=10,height=5)
 grid.newpage();
...
t2 <- t2[,c('label',protIds)]
t2$label[t2$label=='W'] <- 'C'
rownames(t2) <- 1:dim(t2)[1]

p5 <-drawPCA(t2,c(N='#537e35',M='#e17832',A='#f5b819',C='#5992c6',P='red'),colNormalization=T,rowNormalization=T)

pdf("E:/projects/TPD/results/PCA_SWATH_505_protein_20190131_5class.pdf",width=10,height=5)
 grid.newpage();
    pushViewport(viewport(layout = grid.layout(1, 2)));
    print(p2,vp=viewport(layout.pos.row = 1, layout.pos.col = 1))
    print(p3,vp=viewport(layout.pos.row = 1, layout.pos.col = 2))
