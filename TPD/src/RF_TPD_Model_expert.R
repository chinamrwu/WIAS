...
for(i in 1:10){
	nam<-names(RFList)[i]
	pdf(paste0("E:/projects/TPD/results/RF_TPD_expert_ROC_",nam,".pdf"))
	plot.roc(ROCList[[i]],print.auc=T,col = "blue3",print.thres="best",main=names(RFList)[i],legacy.axes = TRUE,print.auc.cex=1.2)
	dev.off()
}

pdf("E:/projects/TPD/results/RF_TPD_expert_ROC_All_in_ONE.pdf")
...
write.table(importantMatrix,file="E:/projects/TPD/results/RF_TPD_expert_top50_important_proteins.txt",sep="\t",col.names=T,row.names=F,quote=F)
