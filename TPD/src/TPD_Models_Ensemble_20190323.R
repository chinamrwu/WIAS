...
for(i in 1:length(models)){
    ...
    response <- predict(models[[i]],newx = as.matrix(tmp1),s = model$lambda.1se,type='response')
	 ...
	 predictions[rownames(pr0),] <- pr0[,1:2] + predictions[rownames(pr0),]
    ...
    predictions[rownames(pr1),] <- pr1[,1:2] + predictions[rownames(pr1),]
...
dat <- ump1$data
wrongA <- rownames(dat)[dat$Y > 0.5 & dat$label=='M']