if(T){
   ...
   set.seed(321)
	...
	C1 <- read.table("data/batchC.txt",sep="\t",header = T,row.names = 1,stringsAsFactors = F)

	Atest <- df0[setdiff(rownames(df0),rownames(A)),]
	Btest <- df0[setdiff(rownames(df0),rownames(B)),]
	Ctest <- df0[setdiff(rownames(df0),rownames(C1)),]
      ...
      while(flg){
		  ...
		  if(length(selection0) >=3){
            ...
            if(length(selection1) >=3){
					...
					acc <- round(100*c(sum(pred==Y1)/dim(VM1)[1],acc),digits=3)
					if(!is.null(VM2)){
							Y2    <- VM2$label
...
R0 <- apply(C1,2,function(v){sum(is.na(v))/length(v)*100})
for(rate in seq(5,100,by=5)){
   for(i in 1:10){
		TM <- PseudoSamples(C1[,R0<=25],weight=1,size=1000)
		VM1 <- PseudoSamples(Ctest,weight=0.9)
		VM2 <- Ctest
		tmp <- glmFeatures1(TM,VM1,VM2)
	}
}
###########################################################
