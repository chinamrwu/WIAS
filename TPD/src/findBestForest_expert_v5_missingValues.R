...
for(lbl in nms){
   ...
   save(bestRFs,file="E:/projects/TPD/data/RF_TPD_Models_expert_v5.Rdata")
   ##########################################  draw ROC plot
	#mt <- matrix(1:10,nrow=2,ncol=5)
	pdf(paste0("E:/projects/TPD/results/RF_TPD_expert_ROC_All_v5_",strTime,".pdf"),width=12,height=5)
	par(mfrow = c(2, 5),mar=c(0,0,0,0))

	for(obj in bestRFs){
		plot.roc(obj$ROC,print.auc=T,col = "blue3",ylim=c(0,1), print.thres="best",asp=NA,
   ...
   write.table(protScore,file=paste0("E:/projects/TPD/results/geneScores_expert_v5_",strTime,".txt"),sep="\t",quote=F,col.names=T,row.names=F)
   ...
   write.table(result,file=paste0("E:/projects/TPD/results/biomarks_expert_v5_",strTime,"_.txt"),sep="\t",quote=F,col.names=T,row.names=F)
