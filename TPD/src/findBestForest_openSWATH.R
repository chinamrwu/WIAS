...
while ( indx < 100 ){
	...
	print(sprintf("Using %d protein to train RF",length(currentFeatures)))
...
pdf("E:/projects/TPD/results/RF_TPD_OpenSWATH_Best_ROC_All_in_ONE.pdf")
...
for(obj in bestRFs){
	imps  <- data.frame(importance(obj$tree));
	impScore <- imps$MeanDecreaseAccuracy * imps$MeanDecreaseGini

	imps <- imps[order(impScore,decreasing=T),]
	imps$protId <- rownames(imps)
	imps$score <- impScore[order(impScore,decreasing=T)]
	imps <-  imps[,c("protId","score")];
	importantProteins <- rbind(importantProteins,imps)
}

t1 <- as.data.frame(t(sapply(unique(importantProteins$protId),function(pid){
score <- sum(importantProteins$score[importantProteins$protId==pid]);c(pid,score)}
)),stringsAsFactors=F)
colnames(t1) <- c("protId","score")
t1$score <- as.numeric(t1$score)
bestProts <- t1[order(t1$score,decreasing=T),]
