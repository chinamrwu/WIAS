...
for(lbl in nms){
   ...
   ##########################################  draw ROC plot
	...
	strTime <- getTime()
	pdf(paste0("E:/projects/TPD/results/RF_TPD_openSWATH_ROC_All_v3_",strTime,".pdf"),width=12,height=5)
   ...
   write.table(protScore,file=paste0("E:/projects/TPD/results/geneScores_OpenSWATH_v3_",strTime,".txt"),sep="\t",quote=F,col.names=T,row.names=F)
   ...
   write.table(result,file=paste0("E:/projects/TPD/results/biomarks_OpenSWATH_v3_",strTime,"_.txt"),sep="\t",quote=F,col.names=T,row.names=F)
